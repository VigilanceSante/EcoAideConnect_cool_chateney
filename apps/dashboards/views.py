...
class DashboardsView(TemplateView):
    def get_context_data(self, **kwargs):
        ...
        pollution_data = self.get_pollution_data()  # Fetch pollution data
        context.update(weather_data)
        context.update(pollution_data)  # Add pollution data to context
        return context

    def get_weather(self):
        url = 'https://api.openweathermap.org/data/2.5/weather?lat=48.7651&lon=2.2666&appid=7b7ff4bb3216cc1bb91bfff3f7ee3f25&units=metric&lang=fr'
        try:
            ...
            wind_direction_cardinal = self.deg_to_cardinal(wind_direction)
            
            # Génération des conseils
            advice = self.get_advice(temp, description, humidity, wind_speed)
    ...
    @staticmethod
    def get_advice(temp, description, humidity, wind_speed):
        advice = []
