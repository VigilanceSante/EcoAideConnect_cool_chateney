...
class WeatherData:
    ...
    def get_weather(self) -> Dict[str, Any]:
        url = f'https://api.openweathermap.org/data/2.5/weather?lat={
            self.lat}&lon={
            self.lon}&appid=7b7ff4bb3216cc1bb91bfff3f7ee3f25&units=metric&lang=fr'
        try:
            ...
            wind_direction_cardinal = WeatherData.deg_to_cardinal(
                wind_direction)

            # Génération des conseils
            advice = WeatherData.get_advice(
    ...
    def get_advice(
        ...
        if temp < -10:
            advice.append(
                "Il fait extrêmement froid aujourd'hui. Portez des vêtements très chauds et évitez les sorties prolongées.")
        elif -10 <= temp < 0:
            advice.append(
                "Il fait très froid. Habillez-vous en plusieurs couches pour rester au chaud.")
        elif 0 <= temp < 10:
            advice.append("Il fait frais. Portez une veste chaude.")
        elif 10 <= temp < 20:
            advice.append(
                "La température est agréable. Une veste légère ou un pull suffira.")
        elif 20 <= temp < 30:
            advice.append(
                "Il fait chaud. Portez des vêtements légers et buvez beaucoup d'eau.")
        elif 30 <= temp < 40:
            advice.append(
                "Il fait très chaud. Restez à l'ombre et buvez beaucoup d'eau.")
        else:
            advice.append(
                "Il fait extrêmement chaud. Prenez des mesures pour vous protéger de la chaleur excessive.")
        ...
        if int(humidity) > 80:
            advice.append(
                "L'humidité est élevée. Vous pourriez vous sentir collant et il pourrait y avoir un risque de moisissure.")
        elif int(humidity) < 30:
            advice.append(
                "L'humidité est basse. Utilisez une crème hydratante pour éviter la sécheresse de la peau.")

        # Conseils basés sur la vitesse du vent
        if wind_speed > 50:
            advice.append(
                "Le vent est très fort. Évitez les activités en extérieur et sécurisez les objets légers.")
        elif wind_speed > 20:
            advice.append(
                "Il y a du vent. Faites attention aux rafales et sécurisez vos affaires.")
...
class PollutionData:
    ...
    def get_pollution_data() -> Dict[str, str]:
        ...
        try:
            ...
            for feature in data.get("features", []):
                ...
                pollution_data['indice'] = properties.get(
                    'indice', 'Non disponible')
                pollution_data['O3'] = properties.get('o3', 'Non disponible')
                pollution_data['NO2'] = properties.get('no2', 'Non disponible')
                pollution_data['PM10'] = properties.get(
                    'pm10', 'Non disponible')
                pollution_data['PM25'] = properties.get(
                    'pm25', 'Non disponible')
                pollution_data['SO2'] = properties.get('so2', 'Non disponible')
                # Since we are interested in the first feature only, we break
                # after the first iteration
                break

            # Convert numeric values to text for all pollutants
            pollution_data = {key: PollutionData.numeric_to_text(
                value) for key, value in pollution_data.items()}
    ...
    def numeric_to_text(value: Any) -> str:
        ...
        return mapping.get(
            value, 'Extrêmement mauvaise' if value not in mapping else 'Non disponible')
